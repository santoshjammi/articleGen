    ...
    try:
        with ftplib.FTP(host) as ftp:
            ...
            if len(found_dirs) >= 2:  # At least 2 of 3 expected directories
                logger.info(f"✅ Remote verification successful: {found_dirs}")
                return True
            else:
                logger.error(f"❌ Remote verification failed: missing expected directories")
                return False
...
def update_manifest(uploaded_files):
    ...
    try:
        ...
        logger.info(f"✅ Manifest updated with {len(uploaded_files)} files")
    except Exception as e:
        logger.warning(f"⚠️ Warning: Could not update manifest: {e}")
...
def upload_folder_parallel(local_dir, remote_dir, max_workers):
    ...
    logger.info("🚀 Starting FTP Sync with Remote Verification")
    logger.info("=" * 50)
    
    verify_start = time.time()
    if not verify_remote_directory(remote_dir, ftp_credentials):
        logger.error("❌ Remote verification failed. Sync aborted.")
        return
    verify_time = time.time() - verify_start
    logger.info(f"✅ Remote verification completed in {verify_time:.2f} seconds")
    ...
    if not remote_files:
        logger.warning("⚠️ Warning: Manifest is empty. This will upload all files.")
...
if __name__ == "__main__":
    ...
    print(f"🚀 Custom FTP Sync (uses manifest from auto_publish.sh) - Log: {log_filename}")
    ...
    try:
        ...
        print(f"✅ Sync completed in {total_time:.1f}s")
